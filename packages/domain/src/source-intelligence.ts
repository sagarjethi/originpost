...
export interface SourceIntelligenceDiscovery {
  ...
  sources: Array<{ title: string; url: string; publisher?: string | undefined; publishedAt?: string | undefined; excerpt?: string | undefined; confidence: number; stableId?: string | undefined; feedUrl?: string | undefined; pageUrl?: string | undefined; snapshot?: SourceEvidence["snapshot"] }>;
...
export function prepareSourceIntelligenceConfig(input: SourceIntelligenceInput): SourceIntelligenceConfig {
  ...
  const sources = input.sources.map((source, index) => {
    ...
    if (source.kind === "public_profile" && source.enabled !== false) throw new DomainError("Social profiles stay off until a dedicated official adapter is configured.", "tracked_source_official_connector_required", 409);
...
export function sourceIntelligenceResearchContext(config: SourceIntelligenceConfig): string {
  ...
  const lumaRules = config.sources.some((source) => source.enabled && source.kind === "luma_city")
    ...
    : "";
  return ["Tracked-source intelligence run. RSS/Atom feeds are machine-readable sources; do not crawl their publisher websites. Publisher sites use the configured browser collector. Public social profiles still require a dedicated provider adapter. Do not bypass login, rate limits, robots controls, or private groups. Do not claim complete coverage.", lumaRules, sources, config.includeTerms.length ? `Prefer items matching: ${config.includeTerms.join(", ")}.` : "", config.excludeTerms.length ? `Exclude items matching: ${config.excludeTerms.join(", ")}.` : ""].filter(Boolean).join("\n");
}
...
export function rankSourceSignals(monitor: MonitorRule, discovery: SourceIntelligenceDiscovery, now = new Date().toISOString()): SourceSignal[] {
  ...
  const matchesTracked = (source: SourceIntelligenceDiscovery["sources"][number], tracked: TrackedPublicSource) =>
    tracked.enabled && ((["public-feeds", "public-tracked"].includes(discovery.provider) && tracked.kind === "rss_atom" && source.feedUrl === tracked.url) || (["public-websites", "public-tracked"].includes(discovery.provider) && tracked.kind === "publisher_site" && source.pageUrl === tracked.url) || urlWithinSource(source.url, tracked));
  for (const suggestion of discovery.suggestions.slice(0, 500)) {
    ...
    const pageEntry = selected.some(source => source.pageUrl) && ["public-websites", "public-tracked"].includes(discovery.provider);
    const eventFingerprint = signalSha256(pageEntry ? { schemaVersion: "originpost.page-event.v1", references: selected.map(source => canonicalPublicUrl(source.url)).sort() } : feedEntry ? { schemaVersion: "originpost.feed-event.v1", references: selected.map(source => canonicalPublicUrl(source.url)).sort() } : { schemaVersion: "originpost.source-event.v1", title: normalizedTitle, references: eventReferences });
...
/** Storage key is derived only from authenticated scope and a content hash. */
export function sourceSnapshotKey(workspaceId: string, brandId: string, sha256: string): string {
  if (!/^[a-f0-9]{64}$/.test(sha256)) throw new DomainError("Invalid source snapshot hash.", "source_snapshot_invalid");
  return `source-snapshots/${encodeURIComponent(workspaceId)}/${encodeURIComponent(brandId)}/${sha256}.png`;
}
