...
export function selectMonitorSourcingProvider(
  ...
  website?: SourcingProvider,
): SourcingProvider {
  const enabled =
    monitor.sourceIntelligence?.sources.filter((source) => source.enabled) ??
    [];
  ...
  const pages = enabled.filter((source) => source.kind === "publisher_site");
  const pageProvider = website;
  ...
  return {
    ...
    async research(request) {
      const providers = [
        ...(feeds.length ? [rss] : []),
        ...(enabled.some((source) => source.kind === "luma_city")
          ? [lumaMumbai]
          : []),
        ...(pages.length && pageProvider ? [pageProvider] : []),
      ];
      const settled = await Promise.allSettled(
        providers.map((provider) => provider.research(request)),
      );
      const results = settled.flatMap((result) =>
        result.status === "fulfilled" ? [result.value] : [],
      );
      if (!results.length)
        throw new Error("All configured public source collectors failed.");
      return {
        provider: "public-tracked",
        model: "public-tracked-v1",
        ...
        toolsUsed: [
          ...results.flatMap((result) => result.toolsUsed),
          ...settled.flatMap((result, index) =>
            result.status === "rejected"
              ? [`collector_failed:${providers[index]!.id}`]
              : [],
          ),
        ],
        raw: {
          collectors: providers.map((provider) => provider.id),
