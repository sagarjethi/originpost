...
export class UpdateAgentRuntimeDto { @IsOptional() @IsString() @MinLength(2) @MaxLength(100) name?:string; @IsOptional() @IsUrl({protocols:["http","https"],require_protocol:true}) @MaxLength(500) baseUrl?:string; @IsOptional() @IsString() @MinLength(1) @MaxLength(160) textModel?:string; @IsOptional() @IsString() @MaxLength(1000) apiKey?:string; @IsOptional() @IsBoolean() disabled?:boolean; }
