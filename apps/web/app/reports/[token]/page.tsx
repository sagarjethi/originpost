...
type ClientReport = {
  ...
  posts: Array<{ brandName: string; title: string; platform: string; accountName: string; liveUrl: string; publishedAt: string; status: string }>;
...
export default function AnalyticsReportPage() {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.main}>
      ...
      <section className={styles.section}><h2>Published posts</h2><p>Every row comes from an exact OriginPost proof of publish. Missing provider data is shown as missing, not zero.</p><div className={styles.posts}>{view.posts.map((post, index) => <article className={styles.post} key={`${post.liveUrl}-${index}`}><div><strong>{post.title}</strong><small>{post.brandName} · {post.platform} · {post.accountName} · {date(post.publishedAt)} · {post.status.replaceAll("_", " ")}</small></div><a href={post.liveUrl} target="_blank" rel="noreferrer">View live <ExternalLink size={13} /></a></article>)}</div></section>
