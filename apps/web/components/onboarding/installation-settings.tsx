...
export function appSetupStatus(view: InstallationView, provider: Provider): string {
  ...
  if (provider === "Images" && view.activeValues.IMAGE_GENERATION_MODE !== "openai") return "Image generation off";
...
export function InstallationSettings({ auth, workspaceId }: { auth: AuthView; workspaceId: string }) {
  ...
  const request = useCallback(async (init: RequestInit = {}) => {
    ...
    try {
      const response = await apiFetch(`/v1/installation/settings?${new URLSearchParams({ workspaceId })}`, { cache: "no-store", ...init, ...(deadline ? { signal: deadline.signal } : {}) }, auth.csrfToken);
      if (!response.ok) {
      ...
      if (deadline?.signal.aborted) throw new Error("The setup server took too long to respond. Check the API connection and reload this page.");
      throw reason;
  ...
  async function removeCredential() {
    ...
    try {
      ...
      setNotice(provider === "Images" ? "Image credentials removed from saved settings. Apply the change to remove them from the running service." : "Service credentials removed from saved settings. Apply the change to remove them from the running service. Connected social accounts must be disconnected separately in Social accounts.");
    ...
    {!owner ? <div className={styles.message}>Only the installation owner can manage server connections. You can continue creating drafts in Agent.</div> : <>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.card}>
          {loading ? <WorkspaceLoading fullPage={false} title="Loading installation" description="Checking your saved settings." /> : !view ? <><h2>Setup could not be loaded</h2><p className={styles.intro}>Check the message above and reload when the API is available. Server settings require the installation owner account.</p><button type="button" className="secondary-button" onClick={() => window.location.reload()}>Reload setup</button><a className="secondary-button" href="/help" target="_blank" rel="noreferrer">Read the installation guide</a></> : <>
            <h2>{steps[step]}</h2>
            ...
            {step === 2 && <><p className={styles.intro}>Connect the services you need for creation or publishing. Keys stay with the installation owner.</p>{provider === "Instagram & Facebook" && <p className={styles.intro}>Use a Facebook Page and its linked professional Instagram account. This setup uses Facebook Login.</p>}<div className={styles.providerTabs}>{providers.map(name => <button key={name} aria-pressed={provider === name} disabled={busy} onClick={() => { if (name !== provider && discardChanges()) setProvider(name); }}>{name === "Images" ? "AI generation" : name}</button>)}</div>{provider === "Images" && <div className={styles.generationGuide}><div><strong>Text &amp; research</strong><p>Connect OpenAI API, local Codex or a compatible text provider, test it, then assign it to your brand. Full news creation also needs live research.</p><a href="/agent-plugins">Configure text providers →</a></div><div><strong>Voice &amp; audio</strong><p>Connect a voice provider and choose language, style and generation limits.</p><a href="/audio">Open audio studio →</a></div></div>}<form key={`${provider}-${view.version}`} onSubmit={save} onChange={() => setDirty(true)}><div className={styles.fields}>
