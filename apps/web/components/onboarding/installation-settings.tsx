...
const steps = ["Your site", "Secure access", "Connect services", "Connect accounts", "Your first post"];
...
export function InstallationSettings({ auth, workspaceId }: { auth: AuthView; workspaceId: string }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setView(next); form.reset();
      setNotice("Settings saved. Saving does not test a connection or publish a post.");
  ...
  async function removeCredential() {
    ...
    try {
      ...
      setNotice("Service credentials removed from saved settings. Apply the change to remove them from the running service. Connected social accounts must be disconnected separately in Channels.");
  ...
  return <section className={styles.page}>
    <WorkspacePageHeader eyebrow="INSTALLATION" title="Make it yours" description="Connect your services, set up your team, and prepare your first post." />
    {!owner ? <div className={styles.message}>Only the installation owner can manage server connections. You can continue creating drafts in Agent.</div> : <>
      ...
      {view?.restartRequired && <div role="status" className={styles.message}>Changes are saved, but are not active yet. Run the installation command on your server: node scripts/install.mjs apply. Then refresh this page.</div>}
      <div className={styles.layout}>
        <nav aria-label="Installation steps" className={styles.steps}>{steps.map((label, index) => <button key={label} type="button" className={styles.step} aria-current={step === index ? "step" : undefined} onClick={() => { setStep(index); setNotice(""); }}><span className={styles.number}>{index + 1}</span>{label}</button>)}</nav>
        <div className={styles.card}>
          {loading ? <WorkspaceLoading fullPage={false} title="Loading installation" description="Checking your saved settings." /> : !view ? <><h2>Sign in securely first</h2><p className={styles.intro}>The server installer creates the first owner account. Sign in with that account to configure services here. Keys are never available to shared creators.</p><a className="secondary-button" href="/help" target="_blank" rel="noreferrer">Read the installation guide</a></> : <>
            ...
            {step === 2 && <><p className={styles.intro}>Add your application credentials here. Meta and Google still require their own developer registration and any applicable review.</p>{provider === "Instagram & Facebook" && <p className={styles.intro}>Facebook Login connects Facebook Pages and their linked Instagram professional accounts. Direct Instagram Login needs a separate integration and is not available in this installer.</p>}<div className={styles.providerTabs}>{providers.map(name => <button key={name} aria-pressed={provider === name} onClick={() => setProvider(name)}>{name}</button>)}</div><form key={`${provider}-${view.version}`} onSubmit={save}><div className={styles.fields}>
              {provider === "Instagram & Facebook" && <>{field("META_APP_ID", "Meta application ID", { required: true })}{field("META_APP_SECRET", "Meta application secret", { required: true, secret: true })}{field("META_WEBHOOK_VERIFY_TOKEN", "Webhook verification token", { secret: true })}{field("META_GRAPH_API_VERSION", "Meta API version", { required: true, hint: "Use a version supported by your Meta application." })}</>}
