...
export function GenerationActions({ text, image, review, research, loading, canConfigure }: { research?: boolean | undefined; text?: boolean | undefined; image?: boolean | undefined; review?: boolean | undefined; loading: boolean; canConfigure: boolean }) {
  const status = (ready?: boolean) => loading || ready === undefined ? 'Checking…' : ready ? 'Configured' : 'Setup needed';
  return <section className={styles.panel} aria-label="Text and image generation">
    ...
    {!loading && canConfigure && (text === false || review === false || image === false) && <div className={styles.actions}>
    ...
    {!loading && !canConfigure && (text === false || image === false || review === false) && <p>Ask your workspace owner to configure the missing services.</p>}
  {!loading && research === false && <p>Full news creation needs live research. Configure the research service on your server; text and image tools can be used separately.</p>}
  </section>;
}
