...
import { AlertTriangle, ArrowRight, Bot, Building2, Cable, Camera, Check, CheckSquare, Clock3, Copy, Database, Download, FileImage, Folder, FolderPlus, HardDrive, LockKeyhole, MessagesSquare, Pencil, Play, Plus, RefreshCw, RotateCcw, Search, Share2, ShieldCheck, Star, Tags, Trash2, Upload, Users, X, Zap } from "lucide-react";
...
export function WorkspaceModules({ module, auth, workspaceId, activeBrandId, brands, creativeContentItemId, onOrganizationChanged, onEngagementUnreadChange, onOpenContent }: { module: WorkspaceModule; auth: AuthView; workspaceId: string; activeBrandId: string; brands: BrandView[]; creativeContentItemId?: string; onOrganizationChanged: (workspaceId?: string) => void | Promise<void>; onEngagementUnreadChange?: (count: number) => void; onOpenContent?: (contentItemId: string) => void }) {
  ...
  if (module === "boards") return <BoardsWorkspace auth={auth} workspaceId={workspaceId} brandId={activeBrandId} />;
...
function MediaLibrary({ auth, workspaceId, brandId }: { auth: AuthView; workspaceId: string; brandId: string }) {
  ...
  return <section className="module-page">
    <div className="module-hero"><div><p className="eyebrow">WORKSPACE MODULE</p><h1>Media library</h1><p>Keep source files, evidence, and creative media private, checked, and connected to the workspace.</p></div><div className="module-actions"><button className="secondary-button" onClick={() => void load()}><RefreshCw size={15} /> Refresh</button>{canUpload ? <button className="new-button" onClick={() => setFormOpen((value) => !value)}><Upload size={15} /> Upload media</button> : null}</div></div>
    ...
    <div className="media-summary-grid" aria-label="Media storage summary">
      <article><span><HardDrive size={17} /></span><div><small>Stored</small><strong>{formatBytes(summary.storedBytes)}</strong></div></article>
      <article><span><FileImage size={17} /></span><div><small>Ready</small><strong>{summary.counts.ready ?? 0}</strong></div></article>
      <article><span><Trash2 size={17} /></span><div><small>In Trash</small><strong>{formatBytes(summary.reclaimableBytes)}</strong></div></article>
      <article><span><Clock3 size={17} /></span><div><small>Unfinished</small><strong>{formatBytes(summary.pendingBytes)}</strong></div></article>
    </div>
