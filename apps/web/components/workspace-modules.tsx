  ...
  return <section className={`module-page${module === "channels" ? " social-accounts-page" : ""}`}>
    ...
    {module === "plugins" ? <>
      {auth.memberships.find(member => member.workspaceId === workspaceId)?.role === "owner"
        ...
        : <div className="module-alert neutral" role="status">Your workspace owner manages AI connections. You can use the approved tools in Agent and Audio without access to provider settings.</div>}
      <div className="module-list panel">
...
function OrganizationModule({ auth, workspaceId, brands, onChanged }: { auth: AuthView; workspaceId: string; brands: BrandView[]; onChanged: (workspaceId?: string) => void | Promise<void> }) {
  ...
  return <section className={`module-page ${organizationStyles.page}`}>
    ...
    <section className="module-list panel" aria-label="Workspace connections">
      <div className="module-list-head"><div><h2>Connections & publishing</h2><p>Choose a brand in the workspace switcher to manage its accounts and providers.</p></div></div>
      <article className="monitor-row"><span className="org-mark small"><Camera size={20}/></span><div><strong>Instagram, Facebook & YouTube</strong><p>Connect through the provider, select your account, then choose it when approving a post. An Instagram handle alone does not grant publishing access.</p></div><a className="secondary-button" href="/channels">Manage channels <ArrowRight size={15}/></a></article>
      <article className="monitor-row"><span className="org-mark small"><ShieldCheck size={20}/></span><div><strong>Voice providers & access</strong><p>Owners manage encrypted API keys, project defaults, and generation limits.</p></div><a className="secondary-button" href="/audio">Audio settings <ArrowRight size={15}/></a></article>
    </section>
