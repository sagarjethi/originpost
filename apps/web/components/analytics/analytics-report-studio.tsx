...
type Snapshot = SnapshotSummary & { reportId: string; reportName: string; groups: Array<{ key: string; brandName: string; platform: string; accountName: string; publishedPosts: number; measuredPosts: number; needsAttention: number; metrics: Metric[] }>; warnings: Array<{ code: string; message: string }>; proofRows: Array<{ title: string }> };
...
const metricOptions = ["views", "engaged_views", "reach", "impressions", "likes", "comments", "shares", "saves", "watch_time_seconds", "average_view_duration_seconds", "subscribers_gained"];
...
export function AnalyticsReportStudio({ auth, workspaceId, activeBrandId, brands }: { auth: AuthView; workspaceId: string; activeBrandId: string; brands: Brand[] }) {
  ...
  async function generate() {
    ...
    setSnapshot(body); setMessage(`Saved an immutable snapshot with ${body.proofRows.length} published post${body.proofRows.length === 1 ? "" : "s"}.`); await loadReports(); await loadDetail(selectedId);
  }
