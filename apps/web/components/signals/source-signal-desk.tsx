...
export function SourceSignalDesk({ auth, workspaceId, brandId, brandName, onOpenContent }: { auth: AuthView; workspaceId: string; brandId: string; brandName: string; onOpenContent?: (contentItemId: string) => void }) {
  ...
  return <section className={styles.page}>
    <header className={styles.hero}>
      <div><p>NEWS & SOURCE DESK</p><h1>Find your next post.</h1><span>Updates from your chosen sources, ready to review and turn into a draft.</span></div>
      <button onClick={() => void load()} disabled={loading}><RefreshCw size={16} className={loading ? styles.spin : ""} /> Refresh</button>
    </header>

    <SourceGenerationGuide brandName={brandName} canConfigure={role === "owner"} />
    ...
    {canManageSources && <details className={styles.feedSetup}>
    ...
    {canManageSources && <details className={styles.preset}><summary>Optional source · Luma Mumbai events</summary><section className={styles.lumaCard} aria-label="Luma Mumbai events">
    ...
    <div className={styles.toolbar}>
      <div>{(["new", "all", "saved", "dismissed"] as const).map((value) => <button key={value} aria-pressed={filter === value} className={filter === value ? styles.active : ""} onClick={() => {setPage(0); setFilter(value);}}>{value === "all" ? "All" : value[0]!.toUpperCase() + value.slice(1)}</button>)}</div>
    ...
    <div className={styles.list}>
      {loading ? <div className={styles.empty}><Loader2 className={styles.spin} /><strong>Loading signals…</strong></div> : shown.map((signal) => <article key={signal.id} className={`${styles.signal} ${styles[signal.urgency]}`}>
        ...
        <div className={styles.actions}>
          {(signal.state === "new" || signal.state === "saved") && <><a className={styles.primary} href={`/agent?sourceSignal=${encodeURIComponent(signal.id)}&sourceVersion=${signal.version}`}>Generate text & image</a><small>Opens Agent with this report and its sources.</small></>}
          {signal.state === "new" ? <><button disabled={busyId === signal.id} onClick={() => void mutate(signal, "save")}><BookmarkPlus size={14} /> Save & research</button><button disabled={busyId === signal.id} onClick={() => void mutate(signal, "dismiss")}><XCircle size={14} /> Dismiss</button></> : null}
          ...
          {signal.state === "saved" && signal.contentItemId && onOpenContent ? <button onClick={() => onOpenContent(signal.contentItemId!)}><BookmarkPlus size={14} /> Open saved content</button> : null}
        </div>
      </article>)}
      {!loading && shown.length === 0 ? <div className={styles.empty}><Radar size={24} /><strong>No {filter === "all" ? "" : filter} signals yet</strong><p>{canManageSources ? "Add your news sources above or adjust the publication window." : "Ask an owner or manager to add sources, or adjust the publication window."} Unknown publication dates appear only in Any time.</p></div> : null}
    </div>
  </section>;
}
