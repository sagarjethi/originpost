...
This is a hidden, version-pinned Hermes 0.21.2 dashboard backend extension. It adds no Hermes tab and is not an OriginPost catalog plugin. OriginPost calls it only from the built-in Hermes panel inside a Board.

Install only after switching the Hermes runtime to a clean Git checkout of exactly `0.21.2` at commit `939e45c91d751fadd94dcd1b873ac3cb44846213`. Keep its Python virtual environment outside that Git checkout. The extension rejects tracked and untracked source changes plus ignored executable Python code/bytecode, including an in-tree virtual environment:
...
Released Board work uses only `POST /tasks/run/{profile}`. OriginPost supplies the immutable task/execution IDs, task content, configuration and capability epochs, enabled-skill names, and the server-owned Profile, memory, and Kanban bindings. The extension rebuilds the sealed policy digest before running. It constructs the execution instructions on the server, exposes no publishing or approval capability, and always returns `outcome: "review"`; a human manager or owner must still approve the result in OriginPost.

Each task execution ID has a durable receipt inside that one Profile. The extension writes an `applying` receipt before calling the model, records the complete attested result atomically, and replays that result without a second model call. If execution stops between those writes, the receipt stays uncertain and the same ID is blocked for operator review rather than run twice.
