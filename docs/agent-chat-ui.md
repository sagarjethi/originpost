...
## Generation entry points — 2026-09-23

The shared **Create** menu opens text + image, text-only, image-only and voice tools from every workspace page. Text/image actions from an opened Content Item retain that item; new text starts with a brief. These are navigation actions, not paid generation requests. Native links preserve setup's unsaved-change warning and reset an existing Agent conversation when starting a new one.

**AI providers** exposes the existing OpenAI API, local Codex and compatible-provider connections. Connect, test, then assign a runtime to the current brand. Image review is a vision capability, separate from image generation. **App setup → AI generation** configures the OpenAI image API and links to text/voice providers. OpenAI API usage has separate billing from ChatGPT; see the official [text guide](https://developers.openai.com/api/docs/guides/text) and [image guide](https://developers.openai.com/api/docs/guides/image-generation). The app continues to use its existing provider adapters; no subscription credentials or unverified provider adapter are added.

For scheduled news updates, open **News sources**, add permitted feeds/pages, then select **Generate text & image** on a report. The existing handoff retains the source/version. Choose a project template and explicitly start creation; research, copy/image review and human publishing approval remain required. Connecting Instagram authorizes publishing to that account, not monitoring arbitrary profiles. Public Instagram profile watching remains unavailable. Topic monitors may produce researched Content Items instead of source-desk leads.

Read-only setup, provider and Agent capability checks now have bounded deadlines and report unavailable status. Paid/mutating requests are not cut off or automatically retried. `node scripts/install.mjs status` checks the API response as well as Docker's listing; a running container alone is not evidence that the application works.

Remaining activation prerequisites: a responsive API and workers, live Hermes or owner-scoped local Codex research, a tested brand text/vision runtime, and either the image API or explicit Codex image upload. Other image providers require an identified API contract and an adapter. These UI changes do not imply that live generation or social publication has been verified on an unconfigured installation.
