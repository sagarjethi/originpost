...
Status: updated 2026-09-15. The `/agent` route implements a durable news-to-image workflow with project templates, exact logo composition, style references, source-desk lead handoff, research, separate copy and image checks, an interactive Codex upload path, and a version-bound `@Publisher` action through the existing approval and scheduling workflow. See [implemented scope and configuration](agent-chat-ui.md). General participant mentions, named multi-agent group chats and additional platform package adaptations remain proposed. A real Codex-image upload has completed source research, copy review, exact composition, vision review and draft creation locally. Live publication remains unverified: the server OAuth apps and a live social account still need configuration, followed by approval of the exact post. The interactive upload path does not require a server image provider.
...
### Codex image handoff implementation checkpoint — 15 September 2026
...
Interactive Codex generation and normal upload have now been exercised with a real source-backed draft. The renderer fits the illustration between the headline/footer panels, and migration 071 stores logo/disclosure/top-headline snapshots in PostgreSQL. A ready composition can be reconnected to an uncertain Codex-upload handoff only when its complete spec, retained image, brief, evidence and passed copy review match. Recovery records the editor and original error, preserves the candidate, and continues through image review. It cannot retry an uncertain provider call or draft write.

Focused API tests pass (36), including interrupted composition, stale-version and changed-copy rejection, no image-generation retry, image review and an unapproved final draft. Two actual PostgreSQL tests pass. The real recovered NASA candidate was stopped by product vision because its illustrated propeller plane was unsupported by the source; no final draft or publication was claimed. A corrected revision is in progress. The desktop admin screenshot remains the only admin screenshot in the README.

### 2026-09-15 — corrected copy to final saved draft
