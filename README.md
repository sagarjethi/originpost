...
Images, video, audio, and PDF files use private object storage. The Library calculates a SHA-256 before upload; the API verifies the stored bytes, streams them through the optional ClamAV gate, and records the scan evidence before marking an asset ready. Live publishing refuses to start unless that gate is enabled. See [docs/media.md](docs/media.md).

Nested brand folders, drag and bulk move, tags, favorites, search, and optimistic organization rules are documented in [docs/media-organization.md](docs/media-organization.md).

Manual native-app publishing and signed external draft review are documented in [docs/review-and-handoff.md](docs/review-and-handoff.md). Set a private `REVIEW_LINK_SECRET` of at least 32 characters before sharing reviewer links.

Instagram, Facebook Page, and YouTube setup, advanced test-account setup, and Connection Doctor are documented in [docs/channels.md](docs/channels.md). Provider tokens are encrypted server-side and are never returned to the browser or audit history.

First Comment drafting, approval, provider permissions, execution fencing, evidence, and uncertainty recovery are documented in [docs/first-comments.md](docs/first-comments.md).

Source-monitor recovery, health states, manual checks, and run history are documented in [docs/monitoring.md](docs/monitoring.md).

Protected workspace health, durable incident transitions, and operator-only alerts are documented in [docs/operations-health.md](docs/operations-health.md).

Ranked source triage, the Luma Mumbai preset, public-host privacy rules, and safe Content Inbox promotion are documented in [docs/source-signal-desk.md](docs/source-signal-desk.md).

Proof-backed candidate selection, fresh-draft preparation, human approval, pause/resume, and recovery rules are documented in [docs/evergreen-refresh.md](docs/evergreen-refresh.md).

Calendar time-zone rules and safe rescheduling are documented in [docs/calendar.md](docs/calendar.md). Brand-wide exact duplicate and same-account timing warnings are documented in [docs/schedule-conflicts.md](docs/schedule-conflicts.md). Notification behavior and deduplication are documented in [docs/notifications.md](docs/notifications.md).

Account-specific weekly clocks, atomic next-slot reservation, DST rules, and cancellation release are documented in [docs/posting-queues.md](docs/posting-queues.md).

The account-scoped Instagram profile projection, coverage limits, and signed preview rules are documented in [docs/instagram-grid.md](docs/instagram-grid.md).

The default is trusted local `single-user` mode. For a team, enable session login and invite members from **Organizations**. Invitations use manually shared, one-time fragment links; this release does not send email. Generic OpenID Connect can use the same sessions and membership rules. See [docs/authentication.md](docs/authentication.md) and [docs/openid-connect.md](docs/openid-connect.md).

Workspace, brand, switching, and resource-isolation rules are documented in [docs/organizations.md](docs/organizations.md).

Official post metrics, refresh jobs, data provenance, and honest missing-data rules are documented in [docs/analytics.md](docs/analytics.md).

Instagram comment sync, webhook setup, human approval, and reply safety are documented in [docs/engagement.md](docs/engagement.md).

Encrypted Instagram and Facebook private-message review, local demo setup, retention, and production gates are documented in [docs/private-conversations.md](docs/private-conversations.md).

External systems, n8n, imports, scoped API keys, and signed lifecycle events are documented in [docs/automation-gateway.md](docs/automation-gateway.md).

Multi-file campaign dry-runs, row recovery, and safe human batch approval are documented in [docs/batch-operations.md](docs/batch-operations.md).

Phone Share Target intake, receipt privacy, deduplication, and browser limits are documented in [docs/mobile-share-capture.md](docs/mobile-share-capture.md).

OpenAI, OpenRouter, Ollama, LM Studio, vLLM, and custom text-runtime setup, encrypted-key handling, private-endpoint gates, brand assignment, and usage records are documented in [docs/agent-runtimes.md](docs/agent-runtimes.md).

Custom image, selected-frame, and Instagram-default Reel covers are documented in [docs/instagram-reel-covers.md](docs/instagram-reel-covers.md).
...
The Compose deployment includes a checksummed PostgreSQL + object-storage backup and disposable restore drill. See [docs/disaster-recovery.md](docs/disaster-recovery.md); an untested dump is not accepted as recovery evidence.

Before a public commit or release, run the complete source, dependency, secret, build, and migration checklist in [docs/public-release.md](docs/public-release.md). Runtime dependency licenses are recorded in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md), and private vulnerability reporting is described in [SECURITY.md](SECURITY.md).
...
The **Agent plugins** screen reads the safe local catalog described in [docs/plugins.md](docs/plugins.md). Discovery validates metadata and requested permissions but does not execute third-party code.
